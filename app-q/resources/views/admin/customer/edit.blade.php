...
<div class="card">
  ...
  <div class="card-datatable table-responsive">
    ...
    <div class="card-header d-flex flex-wrap py-3">
      <form class="ecommerce-customer-add pt-0" style="width: 100%" enctype="multipart/form-data" id="eCommerceCustomerAddForm" method="POST"  action="{{route('customer.update',$show->id)}}">
        @csrf
        <div class="ecommerce-customer-add-basic mb-12">
   
          <div class="mb-12">
            <label class="form-label" for="ecommerce-customer-add-name">العنوان*</label>
            <input type="text" class="form-control" id="ecommerce-customer-add-name" placeholder="العنوان" required value="{{$show->title}}" name="title" aria-label="John Doe" />
 <br>
          
          </div>
          <div class="mb-12">
            <label class="form-label" for="ecommerce-customer-add-email">الوصف*</label>
            <textarea name="description" id="" cols="30" rows="10" class="form-control" required  placeholder="الوصف">{{$show->title}}</textarea>
 <br>
